"""Test that every engine gets properly routed to by LMIE."""
import httpx
import json

LMIE_URL = "http://localhost:8381/query"

# Each test should trigger the target engine based on routing keywords
TESTS = [
...
for target, query in TESTS:
    try:
        with httpx.Client(timeout=10.0) as client:
            # Only check routing, don't wait for full response
            ...
            if resp.status_code == 200:
                ...
                if hit:
                    ...
                    failed += 1
                print(f"  [{status}] {target}: Routed to {routed}", flush=True)
                results.append({"target": target, "status": status, "routed": routed})
            else:
                failed += 1
                print(f"  [FAIL] {target}: HTTP {resp.status_code}", flush=True)
                results.append({"target": target, "status": "FAIL", "http": resp.status_code})
    except Exception as e:
        failed += 1
        print(f"  [FAIL] {target}: {type(e).__name__}: {e}", flush=True)
        results.append({"target": target, "status": "FAIL", "error": str(e)})

print(f"\nROUTING: {passed}/22 engines reachable, {failed} missed", flush=True)

with open("_routing_results.json", "w") as f:
